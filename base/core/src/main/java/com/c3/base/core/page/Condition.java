...
public class Condition<T> implements Specification<T> {
	...
	public Condition<T> companyId(Long companyId) {
		return put("companyId", companyId);
	}

	public Condition<T> deleted() {
		return put("deleted", 1);
	}

	public Condition<T> undeleted() {
		return put("deleted", 0);
	}
